...
uint8_t scan();                                 // код нажатой комбинации (опрашивается в loop)
void status();																	// назначенные пины
...
X3MKeyboard::X3MKeyboard(uint8_t btn0, uint8_t btn1=99) {btn[0]=btn0; pinMode(btn[0],INPUT_PULLUP);
btn[1]=btn1; if(btn1!=99) pinMode(btn[1],INPUT_PULLUP);}
...
void X3MKeyboard::status() {CM("Назначен"); if(btn[1]!=99) CM("ы"); CM(" пин"); if(btn[1]!=99) CM("ы"); CM(" на кнопк");
if(btn[1]!=99) CM("и"); else CM("у"); CM(": "); CM(btn[0]); if(btn[1]!=99) {CM(","); CM(" "); CMn(btn[1]);}}

uint8_t X3MKeyboard::scan() {programmProcess(); if(returnCode) {CM("return from prog start "); CMn(returnCode); uint8_t rCcode=returnCode; returnCode=0; return rCcode;}
// ----------------------------------------------------------------------------------------------------------- фильтр дребезга
for_i(0,2) registr[i]=!digitalRead(btn[i]);
...
if(btState[0] && btState[1]) {mCODE0=3;}} // комбинация кнопок пока не отпустили
...
if(btState[0] && btState[1]) {mCODE1=3;} mCODE0!=mCODE1 ? differentKey=1 : differentKey=0;}
...
if(!progMode){
if(CODE==10 && secondPress && millis()-pressStartTime>3000) {progModeTime=millis(); CODE=0; CMn("Prg start 0"); progMode=1; returnCode=21;} // #1
