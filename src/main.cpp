...
*   Buzer                     Вкл/Откл звуковое сопровождения нажатия клавиш выключателя
...
*   Статус MASTER топика из MQTT:
*   LeftInd                   Вкл/Откл левый индикатор зеленым на выключателях
*   RightInd                  Вкл/Откл правый индикатор зеленым на выключателях
*/
...
#define returnMS(x) \
{static uint32_t tims;\
if (millis() - tims >= x) tims = millis();\
...
#define RGBPIN5 61                              // PIN реле управления RGB подсветкой (Relay(x))
...
//void MQTTRead(char* intopic, char* payload, size_t length);
...
boolean buzer = 1;															// озвучивать события
#endif
#if defined MasterSceneKitchen
...
#define Sync0Ind(x) Led.Sync_I0=x
...
#if defined ButtonEnable
#include "Keyb.h"
X3MKeyboard Button(0,2); // пины под кнопки с замыканием на 0 - 2 кнопки INPUT_PULLUP
...
#if defined RGB_LED_Enable
#include "RGB.h"
XIIIMRGB RGB(5,14,12);                           // PIN ESP Red Greeen Blue (ESP-01s 0,2,3)
#endif
...
CM(">>> Switch Scene "); CM(SCENE); CM(": ");
switch(SCENE) {
case 1: CMn("1 shot"); Buzer(1); break;
case 2: CMn("2 shot"); Buzer(1); break;
case 3: CMn("1+2 shot"); Buzer(1); break;
case 4: CMn("1 long"); Buzer(24); break;
case 5: CMn("2 long"); Buzer(24); break;
case 6: CMn("1+2 long"); Buzer(24); break;
case 7: CMn("1 double"); Buzer(2); break;
case 8: CMn("2 double"); Buzer(2); break;
case 9: CMn("1+2 double"); Buzer(2); break;
case 10: CMn("1 double long"); Buzer(25); break;
case 11: CMn("2 double long"); Buzer(25); break;
case 12: CMn("1+2 double long"); Buzer(25); break;
case 13: CMn("1 → 2 shot"); Buzer(1); break;
case 14: CMn("2 → 1 shot"); Buzer(1); break;
case 15: CMn("1 → 2 long"); Buzer(24); break;
case 16: CMn("2 → 1 long"); Buzer(24); break;
case 17: CMn("1+2 → 1 shot"); Buzer(1); break;
case 18: CMn("1+2 → 1 shot"); Buzer(1); break;
case 19: CMn("1+2 → 2 shot"); Buzer(1); break;
case 20: CMn("1+2 → 2 long"); Buzer(24); break;
case 21: CMn("1 → 1+2 shot"); Buzer(1); break;
case 22: CMn("2 → 1+2 shot"); Buzer(1); break;
case 23: CMn("1 → 1+2 long"); Buzer(24); break;
case 24: CMn("2 → 1+2 long"); Buzer(24); break;}

#if defined ledIndikator_Enable
if(SCENE>20 && SCENE<25) Led.Program_I=1;			// режим программирования
if(SCENE==26 || SCENE==33) Led.Program_I=0;		// программирование отменено || завершено
#endif

if(SCENE>24) return;													// 24 комбинации
...
#if defined MasterSceneLivingRoom // Сцены управления в зале | спальне
switch (SCENE) { // 2,3 зал | 0,1 спальня
case 1: if(!RELAY_Value_Save_[2] && !RELAY_Value_Save_[3]) {RELAY_Value_[2]=1; MQTTStatus("LeftInd",1);} else {RELAY_Value_[2]=0; RELAY_Value_[3]=0; MQTTStatus("LeftInd",0);} break;
case 4: RELAY_Value_[2]=1; MQTTStatus("LeftInd",1); RELAY_Value_[3]=1; MQTTStatus("LeftInd",1); break;
case 7: RELAY_Value_[3]=!RELAY_Value_[3]; if(!RELAY_Value_[2] && !RELAY_Value_[3]) MQTTStatus("LeftInd",0); else MQTTStatus("LeftInd",1); break;
case 2: if(!RELAY_Value_Save_[0] && !RELAY_Value_Save_[1]) {RELAY_Value_[0]=1; MQTTStatus("RightInd",1);} else {RELAY_Value_[0]=0; RELAY_Value_[1]=0; MQTTStatus("RightInd",0);} break;
case 5: RELAY_Value_[0]=1; RELAY_Value_[1]=1; MQTTStatus("RightInd",1); break;
case 8: RELAY_Value_[1]=!RELAY_Value_[1]; if(!RELAY_Value_[1] && !RELAY_Value_[0]) MQTTStatus("RightInd",0); else MQTTStatus("RightInd",1);break;
case 3: case 9: if(!RELAY_Value_Save_[0] && !RELAY_Value_Save_[1] && !RELAY_Value_Save_[2] && !RELAY_Value_Save_[3]) {
RELAY_Value_[0]=1; RELAY_Value_[2]=1; MQTTStatus("LeftInd",1); MQTTStatus("RightInd",1);} else {
RELAY_Value_[0]=0; RELAY_Value_[1]=0; RELAY_Value_[2]=0; RELAY_Value_[3]=0; MQTTStatus("LeftInd",0); MQTTStatus("RightInd",0);} break;
...
#if defined ButtonEnable
char value[50], stat[50], topic[50]; itoa(Button.btn[0], value,10); strcpy(stat, value);
if(Button.btn[1]!=99) strcat(stat,", "); itoa(Button.btn[1], value,10); strcat(stat, value);
strcpy(topic,"Button PIN"); if(Button.btn[1]!=99) strcat(topic,"s"); MQTTStatus(topic, stat); MQTTStatus("Button delay",Button.getCoefficient());
#endif


#if defined BuzerEnable
MQTTStatus("Buzer PIN",Buz.BuzerPIN);
#else
MQTTStatus("Buzer PIN","None");
#endif
#if defined BuzerEnable
save.buzer ? MQTTStatus("Buzer", "ON") : MQTTStatus("Buzer", "OFF");
#endif

#if defined ledIndikator_Enable
#if defined led3ColorIndikator_Enable
MQTTStatus("Indikator LED","3 color");
MQTTStatus("LED Red 1 PIN",Led.red0LED);
MQTTStatus("LED Green 1 PIN",Led.green0LED);
MQTTStatus("LED Blue 1 PIN",Led.blue0LED);
if(Led.red1LED!=99) MQTTStatus("LED Red 2 PIN",Led.red1LED); else MQTTStatus("LED Red 2 PIN","None");
if(Led.green1LED!=99) MQTTStatus("LED Green 2 PIN",Led.green1LED); else MQTTStatus("LED Green 2 PIN","None");
if(Led.blue1LED!=99) MQTTStatus("LED Blue 2 PIN",Led.blue1LED); else MQTTStatus("LED Blue 2 PIN","None");
#else
MQTTStatus("LED Blue PIN",Led.blueLED);
if(Led.redLED!=99) {MQTTStatus("LED Red PIN",Led.redLED); MQTTStatus("Indikator color","2 color");}
else {MQTTStatus("LED Red PIN","None"); MQTTStatus("Indikator color","1 color");}
#endif
#endif
...
if(RELAY_Pin_[i]==RGBPIN5) {strcat(stat,"RGB5 Relay"); itoa(i, value,10); strcat(stat,value);}
...
void MQTTRead(char* intopic, char* payload, AsyncMqttClientMessageProperties properties, size_t length, size_t index, size_t total) {char topic[50], value[50]; char compareStr[50];
#if defined GlobalHouseSwitch
...
if(RELAY_Pin_[i] == RGBPIN0 || RELAY_Pin_[i] == RGBPIN1 || RELAY_Pin_[i] == RGBPIN2 || RELAY_Pin_[i] == RGBPIN3
 || RELAY_Pin_[i] == RGBPIN4 || RELAY_Pin_[i] == RGBPIN5) {RELAY_Value_[i] ? mRGB.powerON(i) : mRGB.powerOFF(i); save.Power[i]=RELAY_Value_[i];}
#endif
...
if(RELAY_Pin_[i] == RGBPIN5) {RELAY_Value_[i] ? mRGB.powerON(5) : mRGB.powerOFF(5); save.Power[i]=RELAY_Value_[i];}
#endif
#if defined Cold_Warm_LED_Enable
...
if (strstr(ReciveTopic,"Status/LeftInd")!=NULL) { // если есть слово Status/LeftInd - включить левый светодиод на выключателе
for_t(0,ctrlNumTopic) {strcpy(compareStr,save.controlTopic[t]); strcat(compareStr,"/Status/LeftInd");
if(!strcmp(ReciveTopic,compareStr)) {SyncInd(1); Led.Lon_I = atoi(ReceiveValue); 
CM("<-из- MQTT:"); CM(ReceiveValue ? " Включаем" : " Выключаем");  CMn(" левый LED"); return;}}}

if (strstr(ReciveTopic,"Status/RightInd")!=NULL) { // если есть слово Status/RightInd - включить левый светодиод на выключателе
for_t(0,ctrlNumTopic) {strcpy(compareStr,save.controlTopic[t]); strcat(compareStr,"/Status/RightInd");
if(!strcmp(ReciveTopic,compareStr)) {SyncInd(1); Led.Ron_I = atoi(ReceiveValue); 
CM("<-из- MQTT:"); CM(ReceiveValue ? " Включаем" : " Выключаем");  CMn(" правый LED"); return;}}}
#endif
...
Comm.status("Controlled topic",t,topic,"plus"); strcpy(topic,save.controlTopic[t]); strcat(topic,"/Status/#"); mqttClient.subscribe(topic,1);
...
#if defined BuzerEnable
if (resiveCommand("Buzer")) {SyncInd(1); CM("<-из- MQTT: Buzer: ");
if(!save.buzer && atoi(ReceiveValue)) {CMn("Включаем"); Buz.en();}
if(save.buzer && !atoi(ReceiveValue)) {CMn("Выключаем"); Buz.dis();} return;}
#endif
...
SwitchScene(); if(SCENE<25) MQTTStatus("SCENE",outValue);}}
#endif
...
if(RELAY_Pin_[i]==RGBPIN4) CM("RGB Module4"); if(RELAY_Pin_[i]==RGBPIN5) CM("RGB Module5"); if(RELAY_Pin_[i]==CWPIN) CM("CWLED");
#if defined RGB_LED_Enable
...
#if defined ledIndikator_Enable
#if defined OTAEnable || defined MQTT_SendEnable
!WiFiOk ? Led.NoWifi_I=1 : Led.NoWifi_I=0;
#if defined MQTT_SendEnable
if(!MQTTOk) Led.NoMQTT_I=1; else Led.NoMQTT_I=0;
if(!Comm.masterSyncOk) Led.NoSync_I=1; else Led.NoSync_I=0;
#endif
#endif
