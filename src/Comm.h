...
// -------------------------------------------------------------------------- Private function -----------------------------------------------------------------------
#if defined MQTT_SendEnable
void sendMQTT(const char* param, const char* value) {char topicSend[50]; strcpy(topicSend,ESPName); strcat(topicSend,"/Status/"); strcat(topicSend,param); mqttClient.publish(topicSend, 1, false, value); CM("-в-> MQTT: "); CM(topicSend); CM(": "); CMn(value);};
void commMQTT(const char* param, const char* value) {mqttClient.publish(param, 1, false, value); CM("-в-> MQTT: "); CM(param); CM(": "); CMn(value);}
void getMQTT();
...
#if defined MQTT_SendEnable
void connectToMqtt() {CMn("\nПодключаемся с MQTT..."); mqttClient.connect();}
void onMqttConnect(bool sessionPresent) {CMn("MQTT сервер подключен"); char topic[50]; strcpy(topic,ESPName); strcat(topic,"/Command/#"); mqttClient.subscribe(topic,1); CM("Подписываемся на: "); CMn(topic);
#if defined ESP_EEPROM_Enable
#if defined GlobalHouseSwitch
mqttClient.subscribe("+/Status/Relay0",1); CMn("Подписываемся на все топики со Status/RelayX");
mqttClient.subscribe("+/Status/Relay1",1);
mqttClient.subscribe("+/Status/Relay2",1);
mqttClient.subscribe("+/Status/Relay3",1);
mqttClient.subscribe("+/Status/Relay4",1);
mqttClient.subscribe("+/Status/Relay5",1);
//mqttClient.subscribe("BedroomCO2/Status/AutoMode",1); CMn("Подписываемся на BedroomCO2/Status/AutoMode");
//mqttClient.subscribe("+/Status/MotionSensor",1); CMn("Подписываемся на все топики со Status/MotionSensor");
mqttClient.subscribe("+/Status/SCENE",1); CMn("Подписываемся на все топики со Status/SCENE");
mqttClient.subscribe("+/Status/IP",1); CMn("Подписываемся на все топики со Status/IP");
if (strcmp(save.controlTopic[0],"")) {strcpy(topic,save.controlTopic[0]); strcat(topic,"/Status/#"); mqttClient.subscribe(topic,1); CM("Подписываемся на: "); CMn(topic);}
//for_i(0,30) {if (strcmp(save.controlTopic[i],"")) {strcpy(topic,save.controlTopic[i]); strcat(topic,"/Command/#"); mqttClient.subscribe(topic,1); CM("Подписываемся на: "); CMn(topic);}}
#else
...
void onMqttDisconnect(AsyncMqttClientDisconnectReason reason) {MQTTOk=0; CMn("\nПропало соединение с MQTT"); if(WiFiOk) mqttReconnectTimer.once(2, connectToMqtt);}

void onMqttMessage(char* topic, char* payload, AsyncMqttClientMessageProperties properties, size_t len, size_t index, size_t total) {
for_i(0, len) ReceiveValue[i] = (char)payload[i]; ReceiveValue[len] = '\0';
CM("=> MQTT: "); CM(topic); CM(" : "); CMn(ReceiveValue);
//MQTTin(topic, payload, len);
}
#endif

void XIIIMCommunicate::getWiFi(uint8_t state=0) {
#if defined OTAEnable
if (WiFiOk && OTApossible) ArduinoOTA.handle();
#endif
static uint32_t wtime, lastScanMillis; static int32_t rssi;
...
if (WiFiOk && state==2) {CMn("\nСоединение WiFi потеряно"); rssi=-100; WiFi.disconnect(); WiFiOk = 0; lastScanMillis=millis()-1900000; return;}
...
if (!firstScan || millis()-lastScanMillis>1800000) { // запускаем сканирование WiFi-сетей:
...
if(!WiFiOk && beginConnect++) {CM("."); lastScanMillis=millis(); if(beginConnect>40) {rssi=-100; lastScanMillis=millis()-1900000;}}}

#if defined MQTT_SendEnable
XIIIMCommunicate::XIIIMCommunicate() {
